...
import { TECHNICAL_TEMPLATES } from './index.ts';
...
import type { CraftDomain } from '../../../types/world.ts';
...
Deno.test('technical: every primitive parameter has at least one template', () => {
	for (const primitive of ALL_PRIMITIVES) {
		...
		for (const parameter of parameters) {
			...
			const found = TECHNICAL_TEMPLATES.some((t) => t.property === id);
			assert(found, `missing template for ${id}`);
		}
	}
});

Deno.test('technical: every decorative technique has at least one template', () => {
...
Deno.test('technical: property ids resolve to a real primitive.parameter or decoration.technique', () => {
	for (const { property } of TECHNICAL_TEMPLATES) {
...
Deno.test('technical: every slot referenced in a template resolves to a real parameter', () => {
...
Deno.test('technical: every variant is register "technical" with empty emphasis', () => {
	for (const { property, variants } of TECHNICAL_TEMPLATES) {
		for (const variant of variants) {
			assertEquals(variant.register, 'technical', property);
			assertEquals(variant.emphasis, [], property);
		}
	}
});

Deno.test('technical: no duplicate property ids', () => {
...
Deno.test('technical: every template has at least one variant with non-empty text', () => {
	for (const { property, variants } of TECHNICAL_TEMPLATES) {
...
Deno.test('technical: conditioned variants precede unconditioned ones within a template', () => {
	for (const { property, variants } of TECHNICAL_TEMPLATES) {
...
Deno.test('technical: every condition.values entry is a real value of its own parameter', () => {
	for (const { property, variants } of TECHNICAL_TEMPLATES) {
...
Deno.test('technical: every condition.craftDomain entry is reachable by at least one shipped material', () => {
	const reachableDomains = new Set<CraftDomain>(MATERIALS.map((m) => m.craftDomain));
	for (const { property, variants } of TECHNICAL_TEMPLATES) {
		for (const variant of variants) {
			for (const domain of variant.condition?.craftDomain ?? []) {
				assert(
					reachableDomains.has(domain),
					`${property}: craftDomain '${domain}' matches no shipped material`,
				);
			}
		}
	}
...
// confirm each value-conditioned structural variant fires on at least one component somewhere in
// it. Decoration's `craftDomain` conditions are guarded separately above (static reachability
// against the material catalogue) rather than through a decoration corpus: no existing dev sampler
// produces `DecorativeLayer` + assigned-material pairs the way `generateArtefact` produces
// structural components, and building one is out of this task's scope.
