...
validates constraints, and then **emits a normalized blueprint output** with parts, slots,
constraints, and per-part materials. The primary path is grammar-first; blueprint-first remains a
compatibility mode for authored artefact types.
...
The grammar-first path **is the primary generation method**. It starts from a context-free grammar
(CFG) and classifies the artefact after structural validation.
