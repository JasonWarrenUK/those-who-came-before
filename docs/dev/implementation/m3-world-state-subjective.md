...
conclusions. This model references artefacts from M1 and provenance from M2, setting up later
contradiction and retcon mechanics.
...
```ts
import type { ArtefactNote, Hypothesis, SubjectiveWorldState } from '$lib/types/subjective';
...
- When creating hypotheses, show **culture** and **period** metadata from the objective world state
