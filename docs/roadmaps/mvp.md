...
- [ ] **2GN.26** — `engine/generation/materials.ts` — `MaterialProvenance` metadata generation
      (source, origin region, trade path) _(depends on 2GN.23 — unblocked)_ — `likelyOriginRegion`
      is necessarily a provisional string keyed against `mockGeologicalContext`'s single
      `'test-region'` fixture (no region vocabulary exists yet — `Culture` carries no region
      binding); true region attribution is 3WS.7's, which must reconcile whatever this task and
      2GN.47 mint (dependency sweep 2026-07-25)
- [ ] **2GN.75** — `engine/generation/materials.ts` —
      ...
      generator — sequenced after 2GN.26 specifically so `provenance` is never stubbed _(blocked —
      depends on 2GN.23, 2GN.26)_ — added by dependency sweep 2026-07-25: 2GN.30/2GN.27/2GN.70 each
...
- [ ] **2GN.27** — `engine/generation/materials.ts` + `engine/generation/classification.ts` —
      ...
      **breaking**: `ExtractedFeatures` carries zero component-material fields today (its one
      ...
      — depends on 2GN.20, 2GN.75)_ — dependency sweep 2026-07-25 corrected the 2GN.23 edge to
...
- [ ] **2GN.35** — `src/lib/data/descriptions/observational/` — observational register templates per
      component type and decorative technique _(depends on 1FD.31, M1)_
