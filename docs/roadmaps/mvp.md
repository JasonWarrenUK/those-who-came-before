...
- [ ] **2GN.48** — `engine/world/scholars.ts` —
      ...
      specialisation, career stage _(depends on 2GN.130, 2GN.66 — all done)_ — 2GN.130 edge added
      2026-08-15: NPC scholars are the first thing that must be named in the _modern_ tongue, not an
      ancient one, so the curated modern language is a naming source this task reads — 2GN.44 edge
      removed by dependency review 2026-07-30: `generateNPCScholars` takes no excavation input, so
      the edge encoded doc 05's stage ordering rather than data flow, and it needlessly gated the
      whole corpus chain behind the description chain — signature takes the loose
      `Culture[]`/`WorldChronology` bag M2 can actually supply rather than `WorldState` (see
