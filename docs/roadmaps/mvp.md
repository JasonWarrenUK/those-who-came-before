...
- [ ] **2GN.36** — `src/lib/data/descriptions/interpretive/` — placeholder interpretive-register
      templates: one placeholder `DescriptionVariant` per property id, full coverage across all 8
      structural primitives and 16 decoration techniques (parity with the shipped `observational/`'s
      39 ids). Each variant: an obviously-fake template string, a single arbitrary `emphasis` tag
      (satisfies the non-empty-emphasis invariant only, not a real framing choice),
      `register: 'interpretive'`, no `condition`. Exists solely to give `2GN.38` real
      `DescriptionTemplate` objects to assemble against _(depends on 1FD.31, M1, 2GN.91 — all done)_
      — **rescoped to placeholder 2026-08-15**: `emphasis`'s selection mechanism (doc 04 §3.4) had
      no defined algorithm anywhere in docs or roadmap when this was first scoped for real
      authoring. Real content, with considered emphasis choices, competing framings and material
      gates, moved wholesale to `6LS.18`, which can be authored against a settled formula
      (`2GN.131`) and live lens state (`6LS.5`) instead of guessing. Do not read this task's
      placeholder prose as design intent. Original dependency rationale still applies: deliberately
      does not depend on 2GN.27/2GN.68 (tag-accumulation changes) — template authoring keys on fixed
      ...
      function/elite/ceremonial tags those tasks compute. Confirmed by roadmap-review (deps, M2)
      2026-08-15
- [ ] **2GN.37** — `src/lib/data/descriptions/technical/` — technical register templates
      (craft-process, manufacturing) _(depends on 1FD.31, M1, 2GN.91 — all done)_ — same rationale
...
- [ ] **2GN.38** — `engine/generation/description.ts` —
      ...
      `prose.ts`'s provisional `variants[0]` indexing (see 2GN.39's note). Depends on `2GN.36`, not
      `6LS.18`: this task assembles the observation list structurally and doesn't need real
      emphasis-authored content to do so — `2GN.36`'s placeholder templates suffice, and `6LS.18`'s
      real content supersedes them later without requiring this task to be redone (added 2026-08-15,
      see `2GN.131`) — `ArtefactPresentation.provenance: ProvenancePresentation` is required, but
      `Provenance` generation is 2GN.47, which sits downstream via `2GN.38 → 2GN.44 → 2GN.47`; per
      dependency sweep 2026-07-25 this task takes a caller-supplied `Provenance` (extract
      `mockArtefact`'s inline default into an exported `mockProvenance` fixture) rather than
      reordering — matches the 2GN.23 M2-provisional convention; revisit once 2GN.47 lands real
      provenance
...
- [ ] **2GN.48** — `engine/world/scholars.ts` —
      ...
      specialisation, career stage _(blocked — depends on 2GN.130; 2GN.66 done)_ — 2GN.130 edge
      added 2026-08-15: NPC scholars are the first thing that must be named in the _modern_ tongue,
      not an ancient one, so the curated modern language is a naming source this task reads — 2GN.44
      edge removed by dependency review 2026-07-30: `generateNPCScholars` takes no excavation input,
      so the edge encoded doc 05's stage ordering rather than data flow, and it needlessly gated the
...
- [ ] **2GN.130** — `src/lib/data/names/modern.ts` — the curated modern language: one pinned
      ...
      belongs to no ancient family and must never be assigned to a culture _(depends on 2GN.66 —
      done; unblocked)_ — ruled during the 2GN.66 spike conversation: the modern language sits
      **outside** `LanguageForest`, not as a family of one inside it, since the forest is
      per-world-seed and generated where this is global and fixed
- [ ] **2GN.131** — `docs/spikes/2GN.131-emphasis-matching.md` + `src/lib/types/lens.ts` — define
      the emphasis-matching formula: how `DescriptionVariant.emphasis` (`ArtefactTag[]`) is scored
      against `LensState.tagWeights` (`Map<ArtefactTag, number>`) to select a variant, filling the
      gap left by doc 04 §3.4 (Descriptive Framing) — the only one of the lens's five channels
      (§3.1–§3.5) with no explicit arithmetic in its own spec, unlike the
      `finalWeight`/`finalScore`/`finalRelevance` pattern the other four already use. Pure design
      decision; produces the formula `6LS.9`'s implementation and `6LS.18`'s authoring both need.
      Does not touch reactive lens state _(depends on 1FD.20, 2GN.91 — both done; unblocked)_ —
      surfaced 2026-08-15 while starting 2GN.36: `DescriptionVariant.emphasis` had a type and a
      stated purpose but no defined scoring mechanism anywhere in docs, roadmap or `docs/spikes/`.
      Confirmed by exhaustive search: no doc 11 deferred-decision entry, no spike file, no task
      notes address it. Deliberately kept out of M6 despite feeding lens-system work, because it's a
      paper formula decision (same status as 2GN.91 defining `VariantCondition`'s shape without
      needing the engine that consumes it) — keeping it in M2 avoids transitively gating `2GN.38`'s
      199 downstream dependents (spanning M3–M10) on the Lens System milestone
...
- [ ] **6LS.9** — `engine/lens/framing.ts` —
      ...
      register + within-register variant _(blocked — depends on 6LS.5, 2GN.131)_ — 2GN.131 edge
      added 2026-08-15: this task implements the emphasis-matching formula, so it needs that formula
      defined first
- [ ] **6LS.18** — `src/lib/data/descriptions/interpretive/` — real interpretive-register authoring,
      replacing `2GN.36`'s placeholders: competing variants per property keyed on genuine
      functional/contextual readings (doc 04 §3.4's own example: one `emphasis: ['weapon']` and one
      `emphasis: ['ceremonial']` framing of the same blade-edge property), `VariantCondition`
      material gates (`craftDomain`/`materialId`/`materialTag`) where material changes the reading,
      respecting the doc 05 §13.1 / doc 12 §2.29 constraint that a `RelativeTag`
      (`elite`/`ceremonial`/`ritual`/`votive`/`funerary`) is never rendered as an intrinsic
      property. Authored against `2GN.131`'s settled matching formula and validated against live
      `LensState` from `6LS.5`; supersedes `2GN.36`'s placeholder content once both land _(blocked —
      depends on 2GN.131, 6LS.5)_
...
graph LR
	...
	2GN.131["2GN.131: `docs/spikes/2GN.131-emphasis-matching…"]
	...
	6LS.18["6LS.18: `src/lib/data/descriptions/interpretive…"]
