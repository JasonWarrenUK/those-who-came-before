...
- [ ] **2GN.8** — `engine/generation/grammar.ts` — normalisation: flatten grammar tree → `NormalisedArtefact` with ordered components, computed dimensions, derived portability _(depends on 2GN.4, 2GN.6)_
- [ ] **2GN.9** — `engine/generation/grammar.ts` — `deriveInspectionDepth(dimensions)` util _(blocked — depends on 2GN.8)_
- [ ] **2GN.10** — `engine/generation/grammar.ts` — `allowedMaterialTags` derivation per component from primitive type + properties compatibility _(blocked — depends on 2GN.8)_
- [ ] **2GN.11** — `src/lib/data/plausibility.ts` — plausibility rule definitions: requires, excludes, ordering, material-physics, ergonomic predicates _(depends on M1)_
- [ ] **2GN.12** — `engine/generation/plausibility.ts` — `checkPlausibility(artefact): { valid, failures }` _(blocked — depends on 2GN.8, 2GN.11, 2GN.1)_
- [ ] **2GN.13** — `engine/generation/plausibility.ts` — physical viability rules (structural integrity, load paths, cantilever limits) _(blocked — depends on 2GN.12)_
- [ ] **2GN.14** — `engine/generation/plausibility.ts` — ergonomic rules (grip length for edged forms, handleability) _(blocked — depends on 2GN.12)_
- [ ] **2GN.15** — `engine/generation/plausibility.ts` — material-structural compatibility (material tags constrain joins/forms) _(blocked — depends on 2GN.12)_
- [ ] **2GN.16** — `engine/generation/plausibility.ts` — re-expansion loop: on failure, re-expand from grammar up to N attempts _(blocked — depends on 2GN.12)_
- [ ] **2GN.17** — `src/lib/data/classification.ts` — classification rules: feature→tag scoring, structural/container/decorative/cross-layer contributions _(blocked — depends on 1FD.12, 2GN.12)_
- [ ] **2GN.19** — `engine/generation/classification.ts` — `extractFeatures(artefact): ExtractedFeatures` — unified feature extraction from components (initially structural features only; 2GN.27 and 2GN.34 complete the doc 05 stage-8 `ExtractedFeatures` contract with material and decorative fields) _(blocked — depends on 2GN.12)_
...
- [ ] **2GN.28** — `src/lib/data/decorations.ts` — decorative technique definitions: surface treatments (polish, patina, scoring, engraving, relief, painting, glaze), applied elements (inlay, overlay, studs, wire-wrapping, gilding), textile elements (wrapping, tassels, beading) with material prerequisites _(depends on 1FD.13, M1)_
- [ ] **2GN.29** — `engine/generation/decoration.ts` — decorative grammar expansion: iterate surfaces, select techniques weighted by culture + phase _(blocked — depends on 2GN.23, 2GN.28)_
- [ ] **2GN.30** — `engine/generation/decoration.ts` — material prerequisite enforcement (engraving → hard material, glaze → ceramic, etc.) _(blocked — depends on 2GN.29)_
- [ ] **2GN.31** — `engine/generation/decoration.ts` — layering support: `DecorativeLayer` with sublayers, decoration-on-decoration _(blocked — depends on 2GN.29)_
- [ ] **2GN.32** — `engine/generation/decoration.ts` — recursion depth cap from `craftSpecialisation` × `aesthetics.decorativeEmphasis` _(blocked — depends on 2GN.29)_
- [ ] **2GN.33** — `engine/generation/decoration.ts` — motif assignment from culture's `motifVocabulary`, shared motifs via cultural exchange _(blocked — depends on 2GN.29)_
...
- [ ] **2GN.58** — Explorer: plausibility panel — generate N structures, show pass/fail with rejection reasons, running rejection rate _(blocked — depends on 2GN.12)_
...
- [ ] **2GN.60** — Explorer: material viewer — resolved material per component, culture bias breakdown (scarcity vs affinity vs trade) _(blocked — depends on 2GN.23)_
- [ ] **2GN.61** — Explorer: decoration inspector — decoration layers per component with prerequisites, technique, layer depth _(blocked — depends on 2GN.29)_
